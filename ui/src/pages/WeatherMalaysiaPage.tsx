...
export function WeatherMalaysiaPage() {
  ...
  async function handleGenerate() {
    ...
    const result = await generate(brand, "single");
  ...
  async function handleScheduleAll(
    ...
    try {
      // FB needs public URLs. The client-rendered canvas is uploaded via the
      // active image provider; Gegar uploads its 3 per-state posters as a carousel.
      ...
      if (isGegar) {
        ...
        const publicId = await uploadToCloudinary(file);
        imageUrls = [uploadedImageUrl(publicId)];
      }
  ...
  return (
    <main className="flex-1 pt-20 md:pt-10 px-4 md:px-8 pb-12 overflow-y-auto">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Post preview — two-column layout */}
        {stage !== 'intro' && (<>
          <div className="mb-8">
