...
const GENERATE_WEBHOOK_URL = import.meta.env
...
export function useArticleGenerator() {
  ...
  const intake = async (
    brand: string,
    links: string[],
  ): Promise<ArticleIntakeResponse | null> => {
    setIsLoading(true);
    setError(null);
    try {
      return await callWebhook<ArticleIntakeResponse>(INTAKE_WEBHOOK_URL, {
        brand,
        links,
      });
    } finally {
  ...
  const generate = async (
    body: Record<string, unknown>,
  ): Promise<ArticleGenerateResponse | null> => {
    setIsLoading(true);
    setError(null);
    try {
      return await callWebhook<ArticleGenerateResponse>(
  ...
  const thumbnailPrompt = async (
    ...
    setError(null);
    try {
      return await callWebhook<ThumbnailPromptResponse>(
        THUMBNAIL_PROMPT_WEBHOOK_URL,
        body,
      );
    } finally {
      setIsLoading(false);
  ...
  const thumbnailGenerate = async (
    ...
    setError(null);
    try {
      return await callWebhook<ThumbnailGenerateResponse>(
        THUMBNAIL_GENERATE_WEBHOOK_URL,
        body,
      );
    } finally {
      setIsLoading(false);
