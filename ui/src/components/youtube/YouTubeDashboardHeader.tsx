...
function CalendarMonth({
  ...
  const handleDayClick = (day: number) => {
    ...
    onSelect(newDate);
    if (!isStart)
      setDisplayMonth(new Date(newDate.getFullYear(), newDate.getMonth(), 1));
  };
    ...
    <div className="w-64">
      ...
      <div className="grid grid-cols-7 gap-1 mb-2">
        {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
          <p
            key={d}
            className="text-xs text-neutral-400 text-center py-1 font-medium"
          >
            {d}
          </p>
        ))}
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div className="relative" ref={brandRef}>
          ...
          >
            {brand}
            <svg
              className={`w-3 h-3 transition ${brandDropdownOpen ? "rotate-180" : ""}`}
              fill="none"
              stroke="currentColor"
              strokeWidth={3}
              viewBox="0 0 24 24"
          ...
          {brandDropdownOpen &&
            (() => {
              ...
              brands.forEach((item) => {
                const entityLabel =
                  BRAND_ENTITY_OVERRIDES[item.label] ||
                  BRAND_ENTITY_OVERRIDES[item.brand] ||
                  BUSINESS_UNIT_LABELS[item.bu] ||
                  "Astro";
                if (!groupedByEntity[entityLabel])
                  groupedByEntity[entityLabel] = [];
                groupedByEntity[entityLabel].push(item);
              });

              Object.values(groupedByEntity).forEach((list) =>
                list.sort((a, b) => a.label.localeCompare(b.label)),
              );
              const entityOrder = knownOrder.filter(
                (k) => groupedByEntity[k]?.length,
              );

              return (
                <div
                  className="absolute top-full left-0 mt-3 z-20 bg-white border border-neutral-200 rounded-lg shadow-xl p-6"
                  style={{ width: "700px" }}
                >
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <div className="flex gap-2 items-center relative" ref={pickerRef}>
            ...
            {pickerOpen && (
              <div
                className="absolute top-full mt-2 z-50 bg-white border border-neutral-200 rounded-lg shadow-xl p-4"
                style={{ right: 0, width: "550px", paddingRight: "24px" }}
              >
