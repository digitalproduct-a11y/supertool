...
const SERIES = [
  { key: 'reactions', label: 'Reactions', color: '#FF3FBF' },
  { key: 'comments', label: 'Comments', color: '#00E5D4' },
          ...
          <Bar dataKey="_anchor" stackId="a" fill="transparent" stroke="none" legendType="none" isAnimationActive={false}>
            <LabelList
              ...
              content={(props: any) => {
                ...
                if (!row) return null
                const activeTotal =
                  (active.has('reactions') ? row.reactions : 0) +
                  (active.has('comments') ? row.comments : 0) +
                  (active.has('shares') ? row.shares : 0)
                if (!activeTotal) return null
                const lbl = activeTotal >= 1000 ? `${(activeTotal / 1000).toFixed(1)}K` : String(activeTotal)
