...
type ToolId = 'home' | 'fb-post' | 'trending-news' | 'affiliate-links' | 'article-generator' | 'engagement-posts' | 'engagement-photos' | 'shopee-top-products' | 'scheduled-posts' | 'post-queue'
      ...
      >
        <div className="w-60 flex flex-col h-full">
              ...
              <div key={i} className={i > 0 ? 'mt-4' : ''}>
                ...
                <div className="space-y-0.5">
                  {group.items.map((tool) => {
                    ...
                    if (tool.comingSoon) {
                      return (
                        <div key={tool.label} className="px-3 py-2.5 rounded-lg text-[13px] font-medium flex items-center gap-2.5 text-neutral-500 cursor-default">
          ...
          <div className="px-3 pb-4 pt-2 space-y-0.5">
            ...
            <p className="px-3 pt-2 text-[11px] text-neutral-400">Made with ♥ by Digital team</p>
          </div>
        </div>
      </aside>

      {showFeedback && createPortal(
        <FeedbackModal activeTool={activeTool} onClose={() => setShowFeedback(false)} />,
        document.body
      )}
    </>
  )
...
function FeedbackModal({ activeTool, onClose }: { activeTool: ToolId; onClose: () => void }) {
    ...
    if (webhookUrl) {
      try {
        await fetch(webhookUrl, {
          ...
          body: JSON.stringify({ name, email, feedback, tool: TOOL_NAMES[activeTool] }),
        })
      } catch { /* fail silently */ }
    }
    setSubmitted(true)
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-neutral-500 mt-0.5">
              {submitted ? "We'll review it and be in touch soon." : "We'd love to hear what's on your mind."}
            </p>
          </div>
        ...
        {submitted ? (
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" placeholder="e.g. Sarah" value={name} onChange={(e) => setName(e.target.value)}
            ...
            <div>
              ...
              <input type="email" placeholder="you@astro.com.my" value={email}
                ...
                className={`w-full px-3 py-2.5 text-sm border rounded-xl focus:outline-none focus:ring-2 focus:border-transparent placeholder:text-neutral-400 ${
                  emailError ? 'border-red-300 focus:ring-red-400' : 'border-neutral-200 focus:ring-neutral-900'
                }`} />
            ...
            <div>
              ...
              <textarea placeholder="What's working well? What could be better? Any bugs?" value={feedback}
            ...
            <button onClick={handleSubmit} disabled={!name.trim() || !isValidEmail || !feedback.trim()}
