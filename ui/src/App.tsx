...
function SuggestButton() {
  ...
  const handleSubmit = async () => {
    const webhookUrl = import.meta.env.VITE_SUGGEST_TOOL_WEBHOOK_URL as
      | string
      | undefined;
    if (webhookUrl) {
      try {
      ...
      {isExpanded && (
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, #FF3FBF, #00E5D4, #0055EE, #F05A35, #FF3FBF, #00E5D4)",
...
function Layout({
  ...
  return (
    <div
      className={`min-h-screen bg-[#f7f7f6] transition-[padding] duration-300 ${isSidebarCollapsed ? "md:pl-0" : "md:pl-60"}`}
    >
      <Sidebar
...
function FbPostPage() {
  ...
  const [captionTitleMode, setCaptionTitleMode] =
    useState<CaptionTitleMode>("original");
  ...
  const [pendingPostArgs, setPendingPostArgs] = useState<PendingArgs | null>(
    null,
  );
    ...
    try {
      const res = await fetch(webhookUrl, {
        ...
        body: JSON.stringify({
          ...
          ...(args.extraPhotos?.length
            ? { uploaded_images: args.extraPhotos }
            : {}),
      ...
      if (data.status === "BRAND_ERROR") {
        return {
          success: false,
          message: data.message ?? "Brand not permitted.",
        };
      }
      if (
        data.success === true ||
        ...
        return {
          success: true,
          message: data.message ?? "Scheduled!",
          postId: data.post_id,
          status: data.status,
        };
      }
  ...
  async function handlePostDraft(
    ...
    return callZernioWebhook(
      { imageUrl, caption, brand, scheduledFor, extraPhotos, postMode },
      creds,
    );
  }
  ...
  const handlePartialRegenerate = useCallback(
    async (
      ...
      const request: WorkflowRequest = {
        ...
        custom_title:
          localTitleMode === "custom" ? localCustomTitle : undefined,
      ...
      if (response.success) {
        const updatedResult = (() => {
          ...
          if (op === "caption_only")
            return { ...result, caption: response.caption } as WorkflowResult;
          return response;
        })();
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-neutral-500 mt-1 text-sm">
                Turn any article into a Facebook image &amp; caption
              </p>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <ol className="space-y-3 list-decimal list-inside text-sm text-neutral-700">
                    <li>
                      <strong>Paste an article URL</strong> — Use "Check
                      supported domains" to see which news sites are compatible
                      before pasting a URL.
                    </li>
                    <li>
                      <strong>Select a brand</strong> — Choose the brand the
                      post is for.
                    </li>
                    <li>
                      <strong>Choose Image Title mode</strong> — Choose whether
                      to use the original article headline, an AI-rewritten
                      title, or a custom title on the image.
                    </li>
                    <li>
                      <strong>Choose Caption Title mode</strong> — Choose
                      whether the caption uses the original article headline or
                      an AI-rewritten version.
                    </li>
                    <li>
                      <strong>Click 'Generate Facebook Post Asset'</strong> —
                      The system will generate the image and caption, which will
                      appear on the right.
                    </li>
                    <li>
                      <strong>Download &amp; copy</strong> — Download the image
                      and copy the caption.
                    </li>
                  </ol>
                </div>
              </div>
                  ...
                  >
                    <path
                      ...
                      style={{
                        ...
                        strokeDashoffset: 30,
                        animation:
                          "draw-check 0.45s cubic-bezier(0.25, 1, 0.5, 1) 0.2s forwards",
                      }}
                    />
                  </svg>
                </div>
...
function CarouselPage() {
  ...
  const carouselWebhookUrl = import.meta.env.VITE_CAROUSEL_WEBHOOK_URL as
    | string
    | undefined;
  ...
  useEffect(() => {
    if (state === "result" && window.innerWidth < 768) {
      setTimeout(() => {
        document
          .getElementById("carousel-preview-panel")
          ?.scrollIntoView({ behavior: "smooth" });
      }, 100);
    }
  }, [state]);
        ...
        <div className="mb-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-neutral-500 mt-1 text-sm">
                Turn any article into a branded photo carousel — 1 main image +
                up to 6 supporting photos
              </p>
            </div>
          </div>
          <div
...
function App() {
  ...
  useEffect(() => {
    ...
    const webhookUrl = (
      import.meta.env.VITE_TRENDING_SPIKE_WEBHOOK_URL as string | undefined
    )?.trim();
    ...
    fetch(webhookUrl, {
      ...
      .then(
        (data: {
          success?: boolean;
          spikes?: Array<{ articleUrl?: string }>;
        }) => {
          if (data.success && Array.isArray(data.spikes)) {
            const seenUrls = getSpikeSeenUrls();
            const todayStr = new Date().toDateString();
            const unread = data.spikes.filter(
              (s: { articleUrl?: string; receivedAt?: string }) => {
    ...
    <>
      ...
      <Routes>
        ...
        <Route
          ...
          element={
            <Layout {...layoutProps}>
              <EngagementPostsLanding
                onSelectTopic={(id) => navigate(topicToPath[id])}
              />
            </Layout>
          }
        />
        <Route
          path="/engagement-posts/epl"
          element={
            <Layout {...layoutProps}>
        ...
        />
        <Route
          path="/engagement-posts/worldcup"
          element={
            <Layout {...layoutProps}>
        ...
        />
        <Route
          path="/engagement-posts/on-this-day-malaysia"
          element={
            <Layout {...layoutProps}>
              <Suspense
                fallback={
                  <div className="flex-1 pt-20 md:pt-10 flex items-center justify-center">
                    <Spinner size="lg" />
                  </div>
                }
              >
